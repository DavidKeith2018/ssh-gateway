...
test('连接诊断：完整步骤、选择账号及指纹失败',async({page})=>{
 ...
 const row=page.getByRole('row').filter({hasText:f.target.name});await row.getByRole('button',{name:'连接诊断'}).click();const d=page.getByRole('dialog',{name:'连接诊断'})
 const account=d.getByLabel('诊断账号');await expect(account).toHaveCSS('appearance','none');await expect(account).toHaveCSS('border-radius','9px');await account.focus();await expect(account).toBeFocused();await account.selectOption('server:default');await d.getByRole('button',{name:'开始诊断'}).click()
 ...
 await page.evaluate(()=>document.documentElement.dataset.theme='dark');await expect(account).toBeVisible();await page.screenshot({path:'test-results/连接诊断-深色选择器.png'});await page.request.delete('/api/targets/'+f.target.id,{data:{}})
})
...
test('连接诊断：取消任务并可再次执行',async({page})=>{
 ...
 await page.goto('/');await page.getByLabel('管理员密码',{exact:true}).fill(f.admin_password);await page.getByRole('button',{name:'进入中转台'}).click();await page.getByRole('row').filter({hasText:f.target.name}).getByRole('button',{name:'连接诊断'}).click()
