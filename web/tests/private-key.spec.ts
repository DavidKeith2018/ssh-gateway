...
for (const encrypted of [false, true]) {
  ...
  await editor.getByRole('button', { name: '显示私钥', exact: true }).click()
    ...
    const showKey = editor.getByRole('button', { name: '显示私钥', exact: true })
    await expect(showKey.locator('svg')).toBeVisible()
    await expect(showKey).toHaveText('')
    await showKey.click()
    await expect(editor.getByLabel('目标私钥', { exact: true })).toHaveValue(target.target_private_key)
    await editor.getByRole('button', { name: '显示私钥密码', exact: true }).click()
    await expect(editor.getByLabel('私钥密码', { exact: true })).toHaveAttribute('type', 'text')
    await expect(editor.getByLabel('私钥密码', { exact: true })).toHaveValue(target.target_key_passphrase || '')
    await editor.getByRole('button', { name: '隐藏私钥', exact: true }).click()
    await expect(editor.locator('.private-key-row textarea')).toHaveCount(0)
    await expect(editor.getByLabel('目标私钥', { exact: true })).toHaveValue('')
    await editor.getByRole('button', { name: '隐藏私钥密码', exact: true }).click()
    await expect(editor.getByLabel('私钥密码', { exact: true })).toHaveAttribute('type', 'password')
    await expect(editor.getByLabel('私钥密码', { exact: true })).toHaveValue('')
